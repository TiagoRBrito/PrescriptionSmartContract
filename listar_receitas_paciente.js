...
async function criarListaReceitas(){
	meu_nome = await contrato.patient_name(await signer.getAddress())
	retorno = await contrato.get_patient_prescriptions();
	...
	for (i=0; i<retorno.length; i++) {
		...
		td_paciente.innerText = receitas.doctor
...
const listarFarmaciasButton = document.querySelector("#listar_farmacias");
async function listar_farmacias(){
	lista_farmacias = await contrato.get_pharmacies();
	nomes = ""
	for (i=0; i<lista_farmacias.length; i++){
		nomes += "\n" + lista_farmacias[i]
	}
	window.alert("As farmácias disponíveis são:" + nomes)
}
listarFarmaciasButton.addEventListener("click", listar_farmacias);

const enviarButton = document.querySelector("#enviar_receita");
async function enviar(){

	const idInput = document.querySelector("#id_receita");
	const nameInput = document.querySelector("#nome_famacia");
	index = idInput.value
	nome_farmacia = nameInput.value

	lista_farmacias = await contrato.get_pharmacies();
	if (! nome_farmacia in lista_farmacias){
		window.alert("Nome da farmácia não encontrada")
	}
	
	retorno = await contrato.get_patient_prescriptions();
	for (i=0; i<retorno.length; i++) {
		if (index == await contrato.get_index(retorno[i])){
			if (await contrato.transfer_to_pharmacy(index, nome_farmacia, Date.now())){
				window.alert("Enviando receita para farmácia")
				criarListaReceitas()
				return
			} else {
	...
	window.alert("ID diferente de uma das suas receitas.")
	return
}
enviarButton.addEventListener("click", enviar);

contrato.on("event_new_prescription", (doctor, patient) => {
	if (patient == meu_nome){
		criarListaReceitas();
	}
})
