...
async function criarListaReceitas(){
	meu_nome = await contrato.doctor_name(await signer.getAddress())
	retorno = await contrato.get_doctor_prescriptions();
	...
	for (i=0; i<retorno.length; i++) {
		...
		td_validade.innerText = (data.getDate() + '/' + month + '/' + data.getFullYear())
...
const repetirButton = document.querySelector("#repetir_receita");
async function repetir(){
	const idInput = document.querySelector("#id_receita");
	const validadeInput = document.querySelector("#validade_receita");
	index = idInput.value
	valid_date = validadeInput.valueAsNumber

	if(valid_date < Date.now()){
		window.alert("Data anterior ao dia de hoje.")
		return
	}
	...
	for (i=0; i<retorno.length; i++) {
		if (index == await contrato.get_index(retorno[i])){
			await contrato.repete_prescription(index, valid_date, Date.now())
			window.alert("Sua receita está sendo criada.")
			return
		}
	}
	window.alert("ID diferente de uma das receitas que criou.")
	return	
}
repetirButton.addEventListener("click", repetir);
