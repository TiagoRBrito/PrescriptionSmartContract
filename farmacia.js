...
const cadastrar = async () => {
	...
	const nameInput = document.querySelector("#nome_cadastro_farmacia");
	...
	farmacias = await contrato.get_pharmacies()
	for (i=0; i<farmacias.length; i++){
		if (register_name == farmacias[i]){
	...
	await contrato.new_pharmacy(register_name);
	cadastroButton.value = "Cadastrar";
...
async function send(){
	...
	if (await contrato.valid_pharmacy(signer.getAddress())) {
		window.location.href = "./listar_receitas_pharmacy.html"
		return
	}
	window.alert("Você ainda não está cadastrado como farmácia, se cadastre antes de acessar essa página.");
}
...
async function gerar_pagina(){
	...
	if (!await contrato.valid_pharmacy(signer.getAddress())) {
		...
		html += '<input type="text" placeholder="Nome de usuário" id="nome_cadastro_farmacia" />'
		html += '<input type="button" value="Cadastrar" id="cadastro_farmacia" />'
		inicio.innerHTML = html
		cadastroButton = document.querySelector("#cadastro_farmacia");
